...
if which node > /dev/null; then
  ...
  printf '\e[1;31m%-6s\e[m\n' "Node.js not found, installing via NVM."
...
if [ -d "$HOME/stacks-blockchain" ]; then
  printf '\e[1;32m%-6s\e[m\n' "stacks-blockchain directory detected."
  # switch to directory
  cd $HOME/stacks-blockchain
  # update from github repo
  git pull
else
  printf '\e[1;31m%-6s\e[m\n' "stacks-blockchain directory not found, cloning."
...
if [ -f "$HOME/keychain.json" ]; then
  printf '\e[1;32m%-6s\e[m\n' "keychain file detected."
else
  printf '\e[1;31m%-6s\e[m\n' "keychain file not found, creating."
...
if curl "https://sidecar.staging.blockstack.xyz/sidecar/v1/faucets/btc/$(jq -r '.keyInfo .btcAddress' $HOME/keychain.json)" | jq -r .balance > 0; then
  printf '\e[1;32m%-6s\e[m\n' "tBTC balance detected. skipping faucet request."
else
  printf '\e[1;31m%-6s\e[m\n' "tBTC balance not found, requesting from faucet."
...
if [ -f $HOME/stacks-blockchain/testnet/stacks-node/conf/argon-miner-conf.toml ]; then
  printf '\e[1;32m%-6s\e[m\n' "Argon config file detected."
else
  printf '\e[1;31m%-6s\e[m\n' "Argon config file not found, downloading."
...
until curl "https://sidecar.staging.blockstack.xyz/sidecar/v1/faucets/btc/$(jq -r '.keyInfo .btcAddress' $HOME/keychain.json)" | jq -r .balance > 0; do
  printf '\e[1;31m%-6s\e[m\n' "tBTC balance not found - checking again in 3min - this is a good time to get coffee!\r\n"
  sleep 180
done

printf '\e[1;32m%-6s\e[m\n' "All checks passed, starting Blockstack Argon miner!"
